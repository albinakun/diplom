...
Процедура ВКМ_ВыполнитьАвтозаполнение() Экспорт
	...
	|	РегистрНакопления.ВКМ_ВыполненныеКлиентуРаботы.Обороты(&НачалоМесяца, &КонецМесяца,, Договор = &Договор
	...
	Если Результат.Пустой() Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон("По договору: %1 клиенту: %2 услуги не оказаны", Договор, Контрагент));
		Возврат;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон("Услуги по договору: %1 заполнены", Договор));			
	КонецЦикла;
